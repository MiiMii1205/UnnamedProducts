...
public static class UnnamedItemPatcher
{
    ...
    public static void StartPostfix(ShittyPiton __instance)
    {
        if (UnnamedPlugin.IsUnnamed(__instance.gameObject))
        {
            ...
            UnnamedPlugin.Log.LogInfo(
                $"Hang time for {__instance.gameObject.name} will be {__instance.totalSecondsOfHang} instead of {prevHangTime} because {__instance.gameObject.name} is UNNAMED");
        }
    }
    ...
    public static void CookingExplodePrefix(ItemCooking __instance, ref bool __runOriginal)
    {
        if (UnnamedPlugin.IsUnnamed(__instance.gameObject))
        {
            ...
            {
                UnnamedPlugin.Log.LogInfo(
                    $"{nameof(UnnamedDynamite)} {__instance.item.gameObject.name} is a dud. Not exploding");
            }
            else if (__instance.item.TryGetComponent(out UnnamedFlare flr) && flr.IsADud)
            {
                UnnamedPlugin.Log.LogInfo(
                    $"{nameof(UnnamedFlare)} {__instance.item.gameObject.name} is a dud. Not exploding");
            }
            else
            {
    ...
    public static void ModifyStatusPrefix(Action_ModifyStatus __instance, ref bool __runOriginal)
    {
        if (UnnamedPlugin.IsUnnamed(__instance.item.gameObject))
        {
            ...
            if (__instance.character is { } chara )
            {
                if (__instance.ifSkeleton && !chara.data.isSkeleton)
                {
                    return;
                }

                var passedOut = chara.data.passedOut;
                ...
                if (__instance.changeAmount < 0f)
                {
                    ...
                    {
                        chara.refs.afflictions.ClearPoisonAfflictions();

                        var poisonPoints = Mathf.RoundToInt(
                            Mathf.Min(
                                chara.refs.afflictions.GetCurrentStatus(CharacterAfflictions.STATUSTYPE
                                    .Poison),
                    ...
                    chara.refs.afflictions.SubtractStatus(__instance.statusType, Mathf.Abs(
                        actualChangeAmount));
                }
                else
                {
                    chara.refs.afflictions.AddStatus(__instance.statusType,
                        Mathf.Abs(actualChangeAmount));
                }

                var statusSum = chara.refs.afflictions.statusSum;
    ...
    public static void BecomeSkeletonPrefix(Action_BecomeSkeleton __instance, ref bool __runOriginal)
    {
        ...
        {
            var shouldTurnEveryoneElse = __instance.item
                .GetData((DataEntryKey) (UnnamedPlugin.UnnamedTotalBaseDataEntryKey + 1), DefaultBookOfBoneBad).Value;
    ...
    public static void ClearAllPrefix(Action_ClearAllStatus __instance, ref bool __runOriginal)
    {
        if (UnnamedPlugin.IsUnnamed(__instance.item.gameObject) && __instance.item.CompareTag("BookOfBones"))
        {
            ...
            if (shouldTurnEveryoneElse)
            {
                ...
                UnnamedPlugin.Log.LogInfo(
                    $"{__instance.item.name} will skeletonize everyone BUT {__instance.character.characterName} because {__instance.item.name} is UNNAMED");
                ...
                foreach (var chara in Character.AllCharacters)
                {
                    ...
                    chara.view.RPC(nameof(UnnamedSkeletonHandler.RPC_ClearAllStatusesBones), RpcTarget.All,
                        __instance.excludeCurse, __instance.otherExclusions.Convert((s) => (int) s).ToArray());
                }
            }
        }
    }

    private static BoolItemData DefaultBookOfBoneBad()
    {
    ...
    {
        if (__instance.character is { } chara && chara.TryGetComponent(out CharacterBurnController burns))
        {
            burns.ExtinguishFires();
        }
    }
    ...
    public static void ClearAllPrefix(Action_ModifyStatus __instance, ref bool __runOriginal)
    {
        ...
        {
            var shouldTurnEveryoneElse = __instance.item
                .GetData((DataEntryKey) (UnnamedPlugin.UnnamedTotalBaseDataEntryKey + 1),
                    DefaultBookOfBoneBad).Value;

            if (shouldTurnEveryoneElse)
            {
                UnnamedPlugin.Log.LogInfo(
                    $"{__instance.item.name} will apply {__instance.changeAmount} of {__instance.statusType} to everyone BUT {__instance.character.characterName} because {__instance.item.name} is UNNAMED");
                ...
                foreach (var chara in Character.AllCharacters)
                {
                    ...
                    chara.view.RPC(nameof(UnnamedSkeletonHandler.RPC_ChangeStatsBones), RpcTarget.All,
                        (int) __instance.statusType, __instance.changeAmount, __instance.ifSkeleton,
    ...
    {
        if (__instance is {statusType: CharacterAfflictions.STATUSTYPE.Hot, changeAmount: < 0f, character: { } chara} &&
            chara.TryGetComponent(out CharacterBurnController burns))
        {
    ...
    public static void LaunchRopePrefix(RopeShooter __instance)
    {
        if (UnnamedPlugin.IsUnnamed(__instance.item.gameObject))
        {
            ...
            UnnamedPlugin.Log.LogInfo(
                $"Setting {nameof(Rope)} length to {__instance.length * mod} instead of {__instance.length} because {__instance.item.name} is UNNAMED");
    ...
    {
        if (UnnamedPlugin.IsUnnamed(__instance.item.gameObject) && Luggage.ALL_LUGGAGE.Count > 0 &&
            __instance.item.inActiveList)
        {
            __runOriginal = false;
            ...
            {
                if (item.TryGetComponent<LuggageBrandHandler>(out var lbh) && lbh.isUnnamed &&
                    item.Center() is var center &&
                    center.SquareDistance(__instance.transform.position) < currentDistance)
                {
        ...
        if (UnnamedPlugin.IsUnnamed(__instance.gameObject))
        {
            ...
            for (var i = 0; i < array.Length; i++)
            {
                ...
                if (character != null && !list.Contains(character))
                {
                    ...
                    var zero = ((!__instance.useSingleDirection)
